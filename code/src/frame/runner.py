...
if __name__ == "__main__":
  ...
  # build the model
  if os.path.exists("./code/data/.ML_model/ARSC_ML_model [0001].ML_model") and os.path.isfile("./code/data/.ML_model/ARSC_ML_model [0001].ML_model"):
    storage.dot_config["data"]["ML_model"] = "./code/data/.ML_model/ARSC_ML_model [0001].ML_model"
    geoDataframe = pd.read_pickle("./code/data/.ML_model/geoDataframe.pickle")
  else:
    geoDataframe = model_builder(file_paths = storage.dot_config["data"]["file_paths"])

  print (f'{datetime.now()} Model Builder complete...')
  ...
  print (f'{datetime.now()} Executing algorithm...')

  speeds_function = generate_speeds_function(min = speed_interval[0], max = speed_interval[1], k = k)

  argument_vector = [storage, costs_function_single_edge_static, is_dominated_function, None, (graph, source_node, destination_node), start_date, speeds_function, geoDataframe, ball_tree]

  _, skyline_routes, _ = algorithms.ARSC_FC.ARSC.ALGORITHM_ARSC().execute_algorithm(argument_vector = argument_vector)[1]

  print (f'{datetime.now()} Appending results...')
